// Left "Index" rail: title-filter field, note-type toggle keys, and a folder tree.
// Notes are drag sources; folders (and the root) are drop zones that move the note
// via `vault.move_note_by_id`. Folder collapse state lives in `collapsed`.
...
impl OmniNoteApp {
    pub fn show_index(&mut self, ctx: &Context) {
        ...
        egui::SidePanel::left("bp_index")
            .resizable(true)
            .default_width(258.0)
            .width_range(196.0..=400.0)
            .show(ctx, |ui| {
                ui.add_space(6.0);
                ui.horizontal(|ui| {
...
fn note_rows(ui: &mut egui::Ui, rows: &[Row], indent: f32, to_select: &mut Option<String>) {
    for (id, label, sel) in rows {
        ui.horizontal(|ui| {
            ...
            ui.dnd_drag_source(egui::Id::new(("bp-note", id)), id.clone(), |ui| {
