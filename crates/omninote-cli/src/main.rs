...
fn main() -> anyhow::Result<()> {
    let cli = Cli::parse();
    let vault_root = resolve_vault(cli.vault.clone())
        .ok_or_else(|| anyhow::anyhow!("no vault: pass --vault, set OMNINOTE_VAULT, or open the GUI once"))?;
    ...
    match cli.command {
        ...
        Command::Note { action } => match action {
            NoteAction::Search { query, json: _ } => {
                anyhow::bail!("note search not implemented yet (CAD-21 Phase B). Query was: {query}")
            }
        },
        Command::Link { action } => match action {
            ...
            LinkAction::Backlinks { file, json: _ } => {
                anyhow::bail!("link backlinks not implemented yet (CAD-21 Phase B). File was: {file}")
            }
        },
    }
