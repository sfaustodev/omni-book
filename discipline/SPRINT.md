...
| 7 | Theme gallery + menu nativo macOS + crash-proofing/formatação/UI polish | ✅ Branch `fix/cad-25-gui-polish` pronta; smoke macOS/visual PASS; sem PR (gate humano) |

> **Estado real (2026-07-12):** 5 de 6 slices originais mergeadas (1-3 #23/#24/#25, Slice 4 #26, **Slice 5 #30**) + ui-polish #28. **CAD-24 Layer A mergeado (#29)**. Falta Slice 6 (`ui_chat` RAG + `ui_dictation`). **Slice 7 nova:** `ThemePreset` 4→9 + menu nativo recebeu crash-proofing, gauntlet 640 células e polish de controles na branch `fix/cad-25-gui-polish`; gates locais verdes (593 passed/1 ignored), clique real do bloco de código com backtrace e visual Almanac/High Contrast passaram, sem PR por gate humano. `QA_FORMATTING.md` separa o executado dos itens manuais restantes. `ui_discipline.rs`/`ui_timeline.rs`/`ui_a11y.rs`/`native_menu.rs` existem; `ui_chat.rs`/`ui_dictation.rs` não. **Sec:** lopdf 0.34→0.42 + quinn-proto 0.11.15 via #27.
