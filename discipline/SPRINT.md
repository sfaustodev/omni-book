...
| 7 | Theme gallery (Almanac/Blueprint/Swiss recuperados de `cortex/off-{1,2}` + stash) + menu nativo macOS (Tema/Editar via `muda`) | 🔄 Implementado, 👀 aguarda teste humano macOS |

> **Estado real (2026-07-10):** 5 de 6 slices originais mergeadas (1-3 #23/#24/#25, Slice 4 #26, **Slice 5 #30**) + ui-polish #28. **CAD-24 Layer A mergeado (#29)** — capture CLI + resolve_active core + --json todos verbos. Falta Slice 6 (`ui_chat` RAG + `ui_dictation`). **Slice 7 nova** (não estava no plano original): `ThemePreset` 4→9 (`native_menu.rs` + `theme.rs`), aguardando smoke test humano em macOS antes de PR (rule #13) — ver `discipline/MANUAL_TEST_PLAN.md`. `ui_discipline.rs`/`ui_timeline.rs`/`ui_a11y.rs`/`native_menu.rs` agora existem; `ui_chat.rs`/`ui_dictation.rs` não. **Sec:** lopdf 0.34→0.42 + quinn-proto 0.11.15 (RUSTSEC-2026-0187/0185) via #27.
